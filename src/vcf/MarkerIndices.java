...
public class MarkerIndices {
     ...
     * @param overlapEnd the end marker index (exclusive) of the overlap
     * with the previous window
     * @param overlapStart the start marker index (inclusive) of the
     * overlap with the next window
     *
     * @throws IndexOutOfBoundsException if
     * {@code overlapEnd < 0 || overlapEnd > inTarg.length}
     * @throws IndexOutOfBoundsException if
     * {@code overlapStart < 0 || overlapStart > inTarg.length}
     * @throws NullPointerException if {@code inTarg==null}
     */
    public MarkerIndices(boolean[] inTarg, int overlapEnd, int overlapStart) {
        if (overlapEnd < 0 || overlapEnd > inTarg.length) {
            throw new IndexOutOfBoundsException(String.valueOf(overlapEnd));
        }
        if (overlapStart < 0 || overlapStart > inTarg.length) {
            throw new IndexOutOfBoundsException(String.valueOf(overlapStart));
        }
        this.prevSplice = overlapEnd>>1;
        this.overlapEnd = overlapEnd;
        this.overlapStart = overlapStart;
        this.nextSplice = (inTarg.length + overlapStart) >>> 1;

        this.targMarkerToMarker = targMarkerToMarker(inTarg);
        this.markerToTargMarker = markerToTargMarker(targMarkerToMarker,
                inTarg.length);

        this.targPrevSplice = targIndex(targMarkerToMarker, prevSplice);
        this.targOverlapEnd = targIndex(targMarkerToMarker, overlapEnd);
        this.targOverlapStart = targIndex(targMarkerToMarker, overlapStart);
        this.targNextSplice = targIndex(targMarkerToMarker, nextSplice);
    }

    /**
     * Constructs a {@code MarkerIndices} instance from the specified data
     * @param overlapEnd the ending marker index (exclusive) for the overlap
     * with the previous window
     * @param overlapStart the starting marker index (inclusive) for the overlap
     * with the next window
     * @param nMarkers the number of markers
     *
     * @throws IllegalArgumentException if {@code nMarkers < 0}
     * @throws IndexOutOfBoundsException if
     * {@code overlapEnd < 0 || overlapEnd > nMarkers}
     * @throws IndexOutOfBoundsException if
     * {@code overlapStart < 0 || overlapStart > nMarkers}
     */
    public MarkerIndices(int overlapEnd, int overlapStart, int nMarkers) {
        ...
        if (overlapEnd < 0 || overlapEnd > nMarkers) {
            throw new IndexOutOfBoundsException(String.valueOf(overlapEnd));
        }
        if (overlapStart < 0 || overlapStart > nMarkers) {
            throw new IndexOutOfBoundsException(String.valueOf(overlapStart));
        }
        this.prevSplice = overlapEnd>>1;
        this.overlapEnd = overlapEnd;
        this.overlapStart = overlapStart;
        this.nextSplice = (nMarkers + overlapStart) >>> 1;
        ...
        this.targPrevSplice = prevSplice;
        this.targOverlapEnd = overlapEnd;
        this.targOverlapStart = overlapStart;
        this.targNextSplice = nextSplice;
    }
    ...
    private static int[] targMarkerToMarker(boolean[] inTarg) {
        IntList il = new IntList(1 + inTarg.length>>6);
     ...
     */
    public int nMarkers() {
        return markerToTargMarker.length;
    }
    ...
    public int nTargMarkers() {
        return targMarkerToMarker.length;
    }
